...
- **Rust SDK behavioral parity with JavaScript-native APIs.** Added semantic lifecycle hooks through `@hook`, the `hooks` module, and `HookEvent`; `astrid:http@1.1.0` per-request deadlines, redirect policy, response/decompression caps, HTTPS-only mode, integrity checks, and response metadata; `astrid:process@1.1.0` read-only child file injection through the discriminated `injectedFiles` option; and schema-versioned KV reads/migrations through a discriminated union. The HTTP module now exports WHATWG-style `fetch` directly, while `process.spawnSync` supplies the familiar Node name without breaking the existing Rust-parity `spawn` helper.
